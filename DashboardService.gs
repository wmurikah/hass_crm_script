...
function getDashboardSummary(affiliateFilter) {
  try {
    ...
    var pendingOrders= orders.filter(function(o){ return ['SUBMITTED','PENDING_APPROVAL','APPROVED'].includes(o.status); });
    ...
    var recentTickets = tickets
      .filter(function(t){ return openStatuses.includes(t.status); })
    ...
    return {
      ...
      recentTickets:  recentTickets.map(function(t){ return { ticket_number:t.ticket_number, subject:t.subject, priority:t.priority, status:t.status }; }),
      recentOrders:   recentOrders.map(function(o){ return { order_number:o.order_number, status:o.status, country_code:o.country_code, total_amount:o.total_amount }; })
    };
  } catch(e) {
